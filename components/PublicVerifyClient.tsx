...
export function PublicVerifyClient({ code, lang }: { code: string; lang: Lang }) {
  ...
  useEffect(() => {
    let live = true;
    (async () => {
      const { data } = await supabase.from("decisions").select("*").eq("code", code.toUpperCase()).eq("status", "APPROVED").maybeSingle();
      if (!live) return;
      setRecord((data || null) as Decision | null); setLoading(false);
    })();
    return () => { live = false; };
  }, [code]);
...
function Verified({record:d,lang}:{record:Decision;lang:Lang}){
  const ar=lang==="ar";
